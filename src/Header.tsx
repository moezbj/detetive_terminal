    ...
    <nav className="sticky top-0 z-50 bg-black/80 backdrop-blur-xl border-b border-white/5 p-4 shadow-2xl">
      <div className={`max-w-6xl mx-auto flex flex-wrap justify-between items-center gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
        <div className={`flex items-center gap-6 ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <Link to="/shop" className={`flex items-center gap-3 bg-neutral-900/50 px-4 py-1.5 rounded-full border border-white/10 hover:bg-neutral-800 transition-all ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <div className={`flex gap-2 bg-neutral-900/40 p-1 rounded-lg border border-white/5 ${isRTL ? 'flex-row-reverse' : ''}`}>
            {['en', 'fr', 'ar'].map((l) => (
              ...
              <button key={l} onClick={() => setLang(l as any)} className={`w-6 h-6 text-[8px] font-black rounded uppercase transition-all ${lang === l ? 'bg-red-700 text-white shadow-lg' : 'text-gray-600 hover:text-gray-400'}`}>{l}</button>
            ))}
          </div>
        </div>
        ...
        <div className={`flex gap-4 items-center ${isRTL ? 'flex-row-reverse' : ''}`}>
          {user.isAdmin && (
             <Link to="/admin" title="Admin Panel" className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-red-500 transition-colors">
                <i className="fa-solid fa-folder-tree"></i>
             </Link>
          )}
          <Link to="/leaderboard" title="Leaderboard" className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-red-500 transition-colors">
            <i className="fa-solid fa-ranking-star"></i>
          </Link>
          <Link to="/profile" title="Profile" className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-red-500 transition-colors">
            <i className="fa-solid fa-user-secret"></i>
          </Link>
          {user.id === 'local_user' ? (
            <Link to="/auth" className="px-4 py-1.5 bg-red-700 text-white rounded font-black text-[10px] uppercase">
              <i className={`fa-solid fa-cloud-arrow-up ${isRTL ? 'ml-2' : 'mr-2'}`}></i> {t.sync}
            </Link>
          ) : (
            <button onClick={logout} className="px-4 py-1.5 bg-neutral-900 border border-white/10 text-gray-500 rounded font-black text-[10px] uppercase">{t.logout}</button>
          )}
        </div>
      </div>
    </nav>
